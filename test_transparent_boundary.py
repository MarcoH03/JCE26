...
def run_boundary_reflection_test(
    ...
    if boundary == "cap":
        ...
        # arXiv:2608.05338 -- see dtbc.py). H here already has bulk on-site
        # 2*t_hop at every site (build_chain_hamiltonian's convention), so --
        # unlike dtbc.py's build_dtbc_matrices, which starts from the
        # bond-summed tools.py convention (on-site only t_lead at the
        # boundary) and must ADD "+t_lead" to reach 2*t_lead -- NO on-site
        # correction is needed here; only the memory/convolution term.
        boundary_label = "exact DTBC (Akramov et al. 2026, Bessel-kernel convolution)"
    elif boundary == "none":
    ...
    tau_c = d.characteristic_time(t_hop, t.h_bar)
    K0 = 1.0 / (2.0 * tau_c)
    if boundary == "dtbc":
        diag_correction = (dt ** 2) * (t_hop ** 2) / (2.0 * t.h_bar ** 2)
        A[n_sites - 1, n_sites - 1] += diag_correction
    A = A.tocsr()
    ...
    if boundary == "dtbc":
        ...
        K_vals = d.bessel_kernel(t_axis, tau_c)
        ...
        for n in range(1, time_steps + 1):
            ...
            source[right_site] = -(dt / t.h_bar) * Src_n
...
def main() -> None:
    ...
    results = {}
    for boundary in ("none", "cap", "transparent", "dtbc"):
    ...
    print(f"Monochromatic self-energy reflected fraction: {r_tbc['reflected_fraction_of_N0']:.6e}")
    print(f"Exact DTBC reflected-back fraction of P0:     {r_dtbc['reflected_fraction_of_N0']:.6e}")
    if r_tbc["reflected_fraction_of_N0"] > 0:
        ...
        print(f"CAP reflects {ratio:.1f}x more probability back into the device than the "
              f"monochromatic self-energy, at the same Fermi energy and packet.")
    if r_dtbc["reflected_fraction_of_N0"] > 0:
        ratio2 = r_cap["reflected_fraction_of_N0"] / max(r_dtbc["reflected_fraction_of_N0"], 1e-300)
        ratio3 = r_tbc["reflected_fraction_of_N0"] / max(r_dtbc["reflected_fraction_of_N0"], 1e-300)
        print(f"CAP reflects {ratio2:.1f}x more probability back than the exact DTBC.")
        print(f"Monochromatic self-energy reflects {ratio3:.1f}x more than the exact DTBC "
              f"(expected: DTBC is exact for the WHOLE wavepacket, not just its central k).")
    print("-" * 78)
