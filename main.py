...
# Chosen to stay below the first left-boundary echo (`~14.22 ps`) while giving
# the packet enough time to clear the ring and populate the right lead.
TOTAL_TIME_PS = 13.5
TIME_STEPS = t.time_steps_for_duration(TOTAL_TIME_PS)
SNAPSHOT_COUNT = 8
...
def compute_wave_number(fermi_energy_mev: float) -> float:
    """Convert the chosen Fermi energy into the incoming lead wave-number."""

    return np.sqrt(2 * t.m * fermi_energy_mev) / t.h_bar


def build_initial_wavefunction(layout: t.SingleRingLayout, k: float) -> np.ndarray:
    """Create a packet that starts in the left lead and moves towards the ring.

    The left lead is stored from the external boundary to the junction. A
    positive `k` therefore points the packet towards the junction and the ring.
    ...
    left_positions_nm = np.arange(layout.left_lead_sites.size, dtype=float) * t.delta_x
    packet_center_nm = INITIAL_PACKET_CENTER_FRACTION * t.L_leads
    packet_width_nm = max(3.0 * t.delta_x, INITIAL_PACKET_WIDTH_NM)

    for site, x_nm in zip(layout.left_lead_sites, left_positions_nm):
        amplitude = np.exp(-0.5 * ((x_nm - packet_center_nm) / packet_width_nm) ** 2) * np.exp(1j * k * x_nm)
...
def build_site_density_history(psi_history: np.ndarray, site_count: int) -> np.ndarray:
    ...
    A local density peak can be greater than two even with only two spin
    components. The bounded quantity is the total discrete probability, not the
    height of the packet at a single node.
    """
...
def build_plot_density_history(site_density_history: np.ndarray, layout: t.SingleRingLayout) -> np.ndarray:
...
def build_probability_history(site_density_history: np.ndarray) -> np.ndarray:
    ...
    The refactor in `tools.py` evolves the state on an orthonormal graph basis.
    In that basis the conserved quantity is the plain discrete norm
    `sum_site,sum_spin |psi|^2`, which is exactly the sum of the local site
    densities returned by `build_site_density_history`.
    """
...
def summarize_probability_regions(final_site_density: np.ndarray, layout: t.SingleRingLayout) -> dict[str, float]:
...
def build_region_probability_history(
    ...
    These region integrals are the most useful diagnostic for a finite-domain
    scattering run: they show whether the packet has actually left the ring and
    separated into transmitted/reflected components before any boundary echo
    ...
    upper_arm_internal = np.sum(site_density_history[:, layout.upper_internal_sites], axis=1, dtype=float)
    lower_arm_internal = np.sum(site_density_history[:, layout.lower_internal_sites], axis=1, dtype=float)
    ...
    ring_total = left_junction + upper_arm_internal + lower_arm_internal + right_junction

    return {
        "left_lead": left_lead,
        "left_junction": left_junction,
        "upper_arm_internal": upper_arm_internal,
        "lower_arm_internal": lower_arm_internal,
...
def estimate_reflection_horizons_ps(k: float) -> dict[str, float]:
    """Estimate when external hard-wall reflections first return to the device.

    The finite leads are a numerical convenience, not true open reservoirs.
    These estimates tell us how long we may trust a run as an effectively
    asymptotic transmission experiment before the outer boundaries contaminate
    the scattering region.
    """

    lead_velocity_nm_ps = t.lead_group_velocity(k)
    packet_center_nm = INITIAL_PACKET_CENTER_FRACTION * t.L_leads
    distance_center_to_left_boundary_nm = packet_center_nm
    distance_center_to_left_junction_nm = t.L_leads - packet_center_nm
    time_to_right_junction_ps = (distance_center_to_left_junction_nm + t.L_ring) / lead_velocity_nm_ps

    return {
        "lead_velocity_nm_ps": lead_velocity_nm_ps,
        "left_reflection_return_to_junction_ps": (
            distance_center_to_left_boundary_nm + t.L_leads
        ) / lead_velocity_nm_ps,
        "right_reflection_return_to_junction_ps": (
            time_to_right_junction_ps + (2.0 * t.L_leads / lead_velocity_nm_ps)
        ),
        "packet_center_to_left_junction_ps": distance_center_to_left_junction_nm / lead_velocity_nm_ps,
        "packet_center_to_right_junction_ps": time_to_right_junction_ps,
    }


def backend_supports_show() -> bool:
    """Return whether the active Matplotlib backend can display interactive windows."""

    return "agg" not in plt.get_backend().lower()


def section_boundaries(section_labels: list[str] | tuple[str, ...]) -> list[float]:
    """Return the x positions where the linearized plot jumps to a new section."""
...
def add_section_guides(axis, section_labels: list[str] | tuple[str, ...]) -> None:
    """Draw light separators between L, U, D and R in the linearized plots."""
...
def sanitize_density_for_plotting(density_history: np.ndarray) -> tuple[np.ndarray, float, int]:
    """Clip non-finite values for plotting without touching the raw saved arrays."""

    finite_mask = np.isfinite(density_history)
    nonfinite_entries = int(density_history.size - np.count_nonzero(finite_mask))

    if np.any(finite_mask):
        finite_values = density_history[finite_mask]
        plot_cap = float(np.quantile(finite_values, 0.995))
...
def save_density_snapshots(
    density_history: np.ndarray,
    snapshot_indices: np.ndarray,
    section_labels: list[str] | tuple[str, ...],
    y_limit_max: float,
    output_path,
) -> None:
    """Save representative density profiles along the linearized graph."""

    x_nodes = np.arange(density_history.shape[1])
    figure, axes = plt.subplots(snapshot_indices.size, 1, figsize=(11, 2.5 * snapshot_indices.size), sharex=True)
...
def save_density_heatmap(density_history: np.ndarray, section_labels: list[str] | tuple[str, ...], output_path) -> None:
    """Save the full space-time density history as a heatmap."""
    ...
    image = axis.imshow(
        ...
        origin="lower",
        cmap="inferno",
        extent=[0, density_history.shape[1] - 1, 0, density_history.shape[0] - 1],
    )
...
def save_potential_profile(
    ...
    """Save the linearized potential profile used during the simulation."""
...
def save_initial_final_density(
    density_history: np.ndarray,
    section_labels: list[str] | tuple[str, ...],
    y_limit_max: float,
    output_path,
) -> None:
    """Compare the first and last density profiles of the run."""
...
def save_probability_history(
    time_axis_ps: np.ndarray,
    probability_history: np.ndarray,
    relative_probability_history: np.ndarray,
    output_path,
) -> None:
    """Save the discrete-probability diagnostic that decides whether the run is sane."""
...
def save_region_probability_history(
    time_axis_ps: np.ndarray,
    region_history: dict[str, np.ndarray],
    output_path,
) -> None:
    """Save the coarse-grained transport observables over time."""
...
def animate_density_history(
    ...
    """Animate the linearized wave-function density over time."""
    ...
    animation_obj = animation.FuncAnimation(
        figure,
        update,
        frames=density_history.shape[0],
        init_func=init,
        blit=True,
    ...
    if save_path is not None:
        ...
        animation_obj.save(save_path, writer=writer)
...
def main() -> None:
    ...
    layout = t.build_single_ring_layout()
    run_directories = rm.create_run_directories()
    execution_started_at = datetime.now().astimezone()
    wall_start = time.perf_counter()

    k = compute_wave_number(FERMI_ENERGY_MEV)
    phase_around_ring = k * (2 * np.pi * t.R)
    lead_velocity_nm_ps = t.lead_group_velocity(k)
    reflection_horizons = estimate_reflection_horizons_ps(k)
    potential_section_summary = t.summarize_potential_by_section()
    print(f"theta = {phase_around_ring:.2f} rad")
    print(f"k = {k:.4f} nm^-1")
    print(f"v_group = {lead_velocity_nm_ps:.2f} nm/ps")
    print(f"potential_model = {t.POTENTIAL_MODEL}")

    A, B, size = t.matrix_A_B_generator_single_ring(t.N_R, k)
    hamiltonian = t.build_single_ring_hamiltonian(layout)

    psi_initial = build_initial_wavefunction(layout, k)
    ...
    density_history_plot, density_plot_cap, nonfinite_density_entries = sanitize_density_for_plotting(plot_density_history)

    node_indices, potential_profile, section_labels = t.build_potential_profile()
    ...
    time_axis_ps = np.arange(TIME_STEPS + 1, dtype=float) * t.dt


    animate_density_history(plot_density_history, section_labels, interval_ms=10, y_limit_max=plot_y_limit)

    save_initial_final_density(
        density_history_plot,
        section_labels,
        plot_y_limit,
        run_directories.images / "density_initial_final.png",
    )
    save_density_snapshots(
        density_history_plot,
        snapshot_indices,
        section_labels,
        plot_y_limit,
        run_directories.images / "density_snapshots.png",
    )
    save_density_heatmap(density_history_plot, section_labels, run_directories.images / "density_heatmap.png")
    save_potential_profile(node_indices, potential_profile, section_labels, run_directories.images / "potential_profile.png")
    ...
    save_region_probability_history(
        time_axis_ps,
        region_probability_history,
        run_directories.images / "region_probabilities.png",
    )

    execution_finished_at = datetime.now().astimezone()
    wall_seconds = time.perf_counter() - wall_start

    hermiticity_residual = hamiltonian - hamiltonian.getH()
    hermiticity_max_abs = float(np.max(np.abs(hermiticity_residual.data))) if hermiticity_residual.nnz else 0.0
    ...
    parameters = t.get_simulation_parameters()
    parameters.update(
        {
    ...
    run_manifest = {
        ...
        "started_at_local": execution_started_at.isoformat(),
        "finished_at_local": execution_finished_at.isoformat(),
        ...
        "potential_model": t.POTENTIAL_MODEL,
        "potential_section_summary": potential_section_summary,
    ...
    rm.write_text(
        ...
        [
            ...
            f"started_at: {execution_started_at.isoformat()}",
            f"finished_at: {execution_finished_at.isoformat()}",
            ...
            f"potential_model: {t.POTENTIAL_MODEL}",
            f"left_reflection_return_to_junction_ps: {reflection_horizons['left_reflection_return_to_junction_ps']:.6f}",
            f"right_reflection_return_to_junction_ps: {reflection_horizons['right_reflection_return_to_junction_ps']:.6f}",
